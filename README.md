...
появляется **новое объявление о продаже квартиры в Минске**, подходящее под
персональный фильтр пользователя (цена, комнаты, площадь, район, этаж),
**мгновенно присылает уведомление в Telegram**.

Главная ценность — скорость: увидеть свежий лот раньше остальных покупателей.

**Многопользовательский:** один бот обслуживает много людей. Каждый пишет
`/start`, настраивает **свой** фильтр (цена, комнаты, площадь, район, этаж) и
получает **свои** уведомления. Площадки опрашиваются один раз на всех; у каждого
подписчика свой фильтр, пауза и персональная история показанных лотов (дедуп на
пользователя). Все функции бесплатны.
...
Город всегда Минск (зашит в конфиг). Остальное настраивается **персонально**
каждым подписчиком прямо в чате и хранится в `users.json`:

- **Цена** — макс. либо диапазон от/до, в USD (`/price`).
- **Комнаты** — одно или несколько допустимых значений (`/rooms 2 3`).
- **Площадь** — диапазон в м² (`/area 40 70`).
- **Район/адрес** — подстрока адреса: район, улица или ЖК (`/district Фрунзенский`).
  Площадки не отдают отдельное поле «район», поэтому фильтр работает по тексту адреса.
- **Этаж** — исключить первый и последний (`/floor`).

Вся проверка лота против фильтра подписчика — в одном методе `users.User.Matches`
(используется и рассылкой, и командой `/last`). Пустое значение фильтра = «не ограничивать».
...
Listing {
  ...
  Rooms      int
  Area       float64  // м²
  Floor      string   // "5/9" — для отображения
  FloorNum   int      // этаж числом (для фильтра «не первый/последний»)
  FloorTotal int      // этажность дома числом
  Address    string
  URL        string
  Photo      string
  CreatedAt  time.Time
...
│   ├── app/                 // сборка и запуск всех компонентов
│   ├── users/               // реестр подписчиков: фильтр, пауза, дедуп (users.json)
│   ├── state/               // статус подключённых площадок (для /sources)
│   ├── telegram/            // отправка по chat_id + приём команд (long polling)
│   └── scheduler/           // опрос площадок + рассылка подписчикам
...
- Отправка объявлений + приём команд (long polling `getUpdates`) в пакете
  `internal/telegram`.
- Формат сообщения: цена, комнаты/площадь/этаж, адрес, площадка, ссылка.
- Если у лота есть фото — отправляется **карточкой с картинкой** (`sendPhoto`)
  с откатом на текст при битой ссылке. Фото есть и у Onliner, и у Kufar
  (`https://rms.kufar.by/v1/gallery/<image.path>`).
...
**Этап 3 — расширенные фильтры и медиа (готово ✅):**
11. ✅ Персональные фильтры: /price (диапазон), /rooms, /area, /district, /floor, /reset.
12. ✅ Фото-карточка объявления (включая Kufar) через sendPhoto.

**Этап 4 (планы):**
13. Коллектор Realt.by (парсинг HTML).
14. SQLite вместо JSON при росте объёма.
15. История цен → алерты о снижении и метка «ниже рынка».
...
| `/status`                | 📊 Статус        | текущие настройки и все фильтры            |
...
| `/price 30000 80000`     | —                | цена: один аргумент = макс, два = диапазон |
| `/rooms 2 3`             | —                | допустимое число комнат                    |
| `/area 40 70`            | —                | диапазон площади, м²                       |
| `/district Фрунзенский`  | —                | подстрока адреса (район/улица/ЖК)          |
| `/floor`                 | —                | исключить первый и последний этаж          |
